...
def get_bucket_key(program: int) -> Optional[int]: 
...
class MidiTokenizer(object):
    ...
    def compile_accompaniment(self, accomp_eventseq: list[int], ticks_per_beat: int, out_path: str) -> miditoolkit.MidiFile:
        ...
        for i,x in enumerate(accomp_eventseq):
        ...
        for k, (switch_ind, program) in enumerate(switches):
            inst_seq: list[int]
            if k != len(switches) - 1:
                inst_seq = accomp_eventseq[switch_ind+1:switches[k + 1][0]]
            else:
                inst_seq = accomp_eventseq[switch_ind+1:]
            ids.append(inst_seq)
...
class MidiExtractor(MidiFileInfo):
    ...
    def extract_other(self) -> list[tuple[int, Optional[int], miditoolkit.Instrument]]:
...
class MidiDatasetTokenizer(object):
    ...
    def write_tokenized(self, dataset_name: str, seq_len: int = 2048, cap: int = 0) -> None:
        '''Given a tokenizer and a directory of midi files, write cleaned data to given output'''

        # Open output file and start counter
        # dataset_path_train: str = f'../data_cleaned/{dataset_name}_train.txt'
        # dataset_path_val: str = f'../data_cleaned/{dataset_name}_val.txt'
        # dataset_path_test: str = f'../data_cleaned/{dataset_name}_test.txt'
        dataset_path_train: str = f'{dataset_name}_train.txt'
        dataset_path_val: str = f'{dataset_name}_val.txt'
        dataset_path_test: str = f'{dataset_name}_test.txt'
        count: int = 0

        f_train = open(dataset_path_train, 'w')
        f_val = open(dataset_path_val, 'w')
        f_test = open(dataset_path_test, 'w')
        ...
        for file in tqdm(self.file_paths):
            ...
            # Check if accompaniment too long
            if len(accomp_tok) > seq_len:
                continue

            # Get other midi info for film
            ticks_per_beat: int = extractor.miditk_obj.ticks_per_beat
            time_sig_arr = extractor.miditk_obj.time_signature_changes
            if time_sig_arr:
                time_sig = '/'.join([
                    str(time_sig_arr[0].numerator),
                    str(time_sig_arr[0].denominator)
                ])
            else:
                time_sig = 'none'

            # Get metadata info for film
            metadata_path: str = str(extractor.metadata_path)
            metadata_file: h5py.File = h5py.File(metadata_path)
            genres = list(map(str, metadata_file['metadata']['artist_terms']))[0:3]
            if len(genres) < 3:
                genres += ['none'] * (3 - len(genres))

            # Store overall film feats
            if str(ticks_per_beat) not in TICKS:
                TICKS.append(str(ticks_per_beat))
            if time_sig not in TIME_SIGS:
        ...
        with open('film_feats.pkl', 'wb') as f:
            pkl.dump([GENRES, TIME_SIGS, TICKS], f)

    def collect_tokenized_lengths(self) -> tuple[list[int], list[int]]:
        '''Given a dataset, return a list of lengths of all tokenized accompaniments.'''

        drum_lengths: list[int] = []
        accomp_lengths: list[int] = []
        ...
        for file in tqdm(self.file_paths):
            ...
            drum_lengths.append(len(drums_tok))
            accomp_lengths.append(len(accomp_tok))

        return drum_lengths, accomp_lengths

    def summarize_film_feats(self, cap: int = 0):

        GENRES: list[str] = ['none']
        TIME_SIGS: list[str] = ['none']
        TICKS: list[str] = ['none']

        count: int = 0
        for file in tqdm(self.file_paths):

            # Get extractor and check validity
            extractor: MidiExtractor = MidiExtractor(file, self.num_accompaniment)
            if (not extractor.has_drums) or (not extractor.miditk_obj.instruments):
                continue

            # Get features from midi file
            ticks_per_beat: str = str(extractor.miditk_obj.ticks_per_beat)
            if ticks_per_beat not in TICKS:
                TICKS.append(ticks_per_beat)
            if extractor.miditk_obj.time_signature_changes:
                time_sig: str = '/'.join(
                    [
                        str(extractor.miditk_obj.time_signature_changes[0].numerator),
                        str(extractor.miditk_obj.time_signature_changes[0].denominator)
                    ]
                )
                if time_sig not in TIME_SIGS:
                    TIME_SIGS.append(time_sig)
            ...
            metadata_file: h5py.File = h5py.File(str(extractor.metadata_path))
            genres: list[str] = list(map(str, metadata_file['metadata']['artist_terms']))[0:3]
            for genre in genres:
                if genre not in GENRES:
                    GENRES.append(genre)
